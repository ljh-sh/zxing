...
//!
//! Hand-rolled (no clap) to keep the dependency tree minimal and the
//! static binary small. The argument form is:
//!
//! ```text
...
pub const VERSION_INFO: &str = concat!(
    ...
    " — local QR + 1D barcode decoder CLI (Rust wrapper around ZXing)"
);
