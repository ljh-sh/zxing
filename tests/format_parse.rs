...
#[test]
fn json_emits_valid_shape_with_points_always_included() {
    let mut buf = Vec::new();
    let results = vec![zxing::Decoded {
    ...
    zxing::emit_json(&mut buf, Path::new("ean.png"), &results);
    let s = String::from_utf8(buf).unwrap();
    // The `points` field is always emitted (no `--points` flag in v0.2+).
    // An empty Vec renders as `"points": []` to keep the schema stable.
    assert_eq!(
        s,
        "[{\"file\": \"ean.png\", \"results\": [{\"format\": \"EAN_13\", \"text\": \"4006381333931\", \"points\": []}]}]\n"
    );
}

#[test]
fn json_includes_corner_points_when_available() {
    let mut buf = Vec::new();
    let results = vec![zxing::Decoded {
        format: BarcodeFormat::QR_CODE,
        text: "hi".to_string(),
    ...
    zxing::emit_json(&mut buf, Path::new("qr.png"), &results);
    let s = String::from_utf8(buf).unwrap();
    // Snapshot test — exact byte-for-byte output. Update when the
    // format intentionally changes.
    assert_eq!(
        s,
        "[{\"file\": \"qr.png\", \"results\": [{\"format\": \"QR_CODE\", \"text\": \"hi\", \"points\": [[10.0, 10.0], [200.0, 10.0], [200.0, 200.0], [10.0, 200.0]]}]}]\n"
    );
}
...
fn tsv_escapes_tabs_and_newlines_in_text_field() {
    ...
    zxing::emit_tsv(&mut buf, Path::new("qr.png"), &results);
    let s = String::from_utf8(buf).unwrap();
